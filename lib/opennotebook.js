...
async function verifyConnection() {
  ...
  try {
    // OpenNotebook doesn't have /api/health, so check /api/notebooks instead
    const response = await fetch(`${OPENNOTEBOOK_URL}/api/notebooks`, {
      method: 'GET',
    ...
    if (response.ok) {
      ...
      logger.warn('OpenNotebook not available', { url: OPENNOTEBOOK_URL, status: response.status });
    }
  } catch (error) {
    connectionFailed = true;
...
export async function syncCircuitState(spokeName, state) {
  ...
  try {
    ...
    await callAPI(`/api/notes/${CIRCUIT_BREAKER_NOTE_ID}`, 'PUT', {
...
export async function updateWorkflowStatus(notebookId, status, summary = {}) {
  ...
  try {
    await callAPI(`/api/notebooks/${notebookId}`, 'PUT', {
      description: JSON.stringify({
        ...summary,
        status,
